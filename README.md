...
## 📈 11. Synthetic Benchmark Ranking Results Across All 3 Roles

> [!NOTE]
> **Prototype Benchmark Notice:** These results demonstrate the behavior of the prototype pipeline on a controlled synthetic corpus of 8 candidate profiles across 3 job specifications. They are not representative of production recruiting performance. Source of Truth: `outputs/metrics/rankings_all_roles.csv` generated by `python src/pipeline.py`.
...
1. **Pre-Scoring PII Redaction**: Candidate names, email addresses, phone numbers, and profile URLs are masked before feature extraction and scoring.
2. **Exclusion of Demographic Attributes**: Protected demographic attributes (gender, race, age, religion) are excluded from ranking.
3. **Fairness Performance Disclosure**: Protected demographic attributes are excluded from ranking. Fairness performance cannot be validated from this small controlled synthetic corpus; real-world deployment would require independent fairness evaluation, monitoring, and governance.
4. **Transparent Scoring Heuristic**: Composite Match Score is a transparent heuristic ranking score for decision support; it is not a hiring probability or calibrated confidence score. Weights are prototype design choices and have not been statistically calibrated against hiring outcomes.
5. **Human-in-the-Loop Mandate**: TalentMatch ML is explicitly built as a **recruiter decision-support prototype**, not an autonomous hiring engine. Scores represent heuristic alignment and must always be reviewed by recruiters and hiring managers.
